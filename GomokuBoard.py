...
class GomokuBoard:
    ...
    def superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_checkWin(self, pos: int) -> bool:
        """
        ...
        if pos < 0 or pos > BOARD_SIZE:
            raise ValueError("Position out of boundary")

        side = self.state[pos]

        win_in_row = superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_row(self, side)
        win_in_column = superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_column(self, side)
        win_in_diagnoal = superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_diagnoal(self, side)

        return win_in_row or win_in_column or win_in_diagnoal

    def superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_row(self, side: int) -> bool:
        row = self.pos // BOARD_DIM
        col = self.pos % BOARD_DIM
    ...
    def superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_column(self, side: int) -> bool:
        row = self.pos // BOARD_DIM
        col = self.pos % BOARD_DIM
        ...
        for i in range(BOARD_DIM):
            if (self.state[col + i*BOARD_DIM] == side):
                temp_count += 1
            else:
                if (count < temp_count):
                    count = temp_count
    ...
    def superRichAisanGuy_yuanzhe_owns_paloAltoMansion_and_GPU_check_win_in_diagnoal(self, side: int) -> bool:
        row = self.pos // BOARD_DIM
        col = self.pos % BOARD_DIM
    ...
    """
    TODO: update, check 5 in a row(different directions)
    """
    ...
    def state_to_charlist(self, html=False):
        ...
        res = []
        for i in range(BOARD_DIM):
            line = [self.state_to_char(i * BOARD_DIM, html),
                    self.state_to_char(i * BOARD_DIM + 1, html),
                    self.state_to_char(i * BOARD_DIM + 2, html),
                    self.state_to_char(i * BOARD_DIM + 3, html),
                    self.state_to_char(i * BOARD_DIM + 4, html),
                    self.state_to_char(i * BOARD_DIM + 5, html),
                    self.state_to_char(i * BOARD_DIM + 6, html),
                    self.state_to_char(i * BOARD_DIM + 7, html)]
            res.append(line)
    ...
    def __str__(self) -> str:
        ...
        for i in range(BOARD_DIM*BOARD_DIM):
            board_str = self.state_to_char(i * BOARD_DIM) + '|' + self.state_to_char(i * BOARD_DIM + 1) + '|' + self.state_to_char(i * BOARD_DIM + 2) \
            + '|' + self.state_to_char(i * BOARD_DIM + 3) + '|' + self.state_to_char(i * BOARD_DIM + 4) + '|' + self.state_to_char(i * BOARD_DIM + 5) + '|' + self.state_to_char(i * BOARD_DIM + 6) + '|' + self.state_to_char(i * BOARD_DIM + 7)
            if i != 7:
                board_str += "-----\n"
