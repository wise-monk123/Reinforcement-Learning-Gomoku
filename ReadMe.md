...
Yuanzhe Li
Samuel Yang 
This report is prepared for CMPE 297-99 instructed by Professor Jahan Ghofraniha 
Table of Content:
Executive Summary
...
Conclusion:
We started from a Monte Carlo Tree Search algorithm for Gomoku, and improved it into Q learning deep network and trained through colab GPU. Furthermore we fine tuned the hyperparameter in order to improve the performance. Please refer to the colab ipython notebook on project github. We experimented on combination battles between Min Max Algorithm, tabular reinforced learning algorithm , neural network based algorithm as well as the policy gradient approach. The Min-Max algorithm can easily beat the random approach, while the tabular Q-learning player learns to play optimally against the Min-Max in less than 100 games. We built a QNetwork at first but we found that when QNetwork battle with Min-Max, it is either 100% draw or 100% Loss. So we fine tuning the QNetwork by experiments on the epsilon, optimizer learning rate, reward discount, win/loss/draw rewards, and then we also use policy gradient descent to better learn the Q function.Finally we were able to achieve a noticeable improvement on the performance.
Reference:

“The History of Gomoku.” Home, http://gomokuworld.com/gomoku/3.
